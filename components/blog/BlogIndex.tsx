          ...
          <ul className="divide-y divide-white/10 border-y border-white/10">
            {posts.map((post) => (
              <li key={post.slug}>
                <Link
                  href={`/blog/${post.slug}`}
                  className="group flex flex-col gap-5 py-8 transition-colors sm:flex-row sm:gap-7 sm:items-start"
                >
                  {post.image ? (
                    ...
                    <img
                      src={post.image}
                      alt={post.imageAlt ?? ''}
                      className="w-full aspect-square object-cover border border-white/10 bg-white/[0.02] sm:w-[200px] sm:shrink-0"
                    />
                  ) : null}
